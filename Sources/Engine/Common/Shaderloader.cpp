...
int ShaderLoader::Load(std::string pathToIndexer){

	std::string index_file_path;
	if(SHADER_USE_PASSED_EXE_PATH == 0){
		index_file_path = "src\\Engine\\Shaders\\shaders_indexer.txt";
	} else if(SHADER_USE_PASSED_EXE_PATH == 1){
		if(pathToIndexer.size()>0){
			index_file_path = pathToIndexer.substr(0, pathToIndexer.rfind('\\'))+"\\Shaders\\shaders_indexer.txt";
		}
	}
    ...
    std::cout<<"Shader indexer file: "<<index_file_path<<std::endl;
    if( index_file.good() ){
        std::string line;
        std::string path;
        while( !index_file.eof() )
        {
            ...
            if (found!=std::string::npos){
            	...
            	if(SHADER_USE_PASSED_EXE_PATH == 0){
            		value = CompileShader(path+file_path);
            	} else if(SHADER_USE_PASSED_EXE_PATH == 1){
            		value = CompileShader(pathToIndexer.substr(0, pathToIndexer.rfind('\\'))+"\\"+path+file_path);
            	}
...
//            				std::string param_name = params.substr(0,start);
            				...
            				//std::cout<<"Shader "<<value<<"    Getting: "<<param_ref_name<<"   Get: "<<temp<<std::endl;
            				std::cout<<"Parametr: "<<param_ref_name.c_str()<<" Wartosc: "<<temp<<std::endl;
            				refs.push_back(temp);
            	...
            	std::cout<<"Program name?: "<<key<<std::endl;
